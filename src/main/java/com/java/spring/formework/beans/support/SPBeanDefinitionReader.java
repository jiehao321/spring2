...
public class SPBeanDefinitionReader {
    ...
    public SPBeanDefinitionReader(String... locations){
    ...
    //把配置文件中扫描到的所有配置信息转换为SPBeanDefinition对象，以便于之后的ioc操作方便
    public List<SPBeanDefinition> loadBeanDefinitions(){
        List<SPBeanDefinition> result = new ArrayList<SPBeanDefinition>();
        try {
    ...
    private SPBeanDefinition doCreateBeanDefinition(String factoryBeanName,String beanClassName){
    ...
    private SPBeanDefinition doCreatBeanDefinition(String className) {
        try {
            Class<?> beanClass = Class.forName(className);
            //如果不是一个接口，就new
